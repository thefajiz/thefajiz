...
interface QuizQuestion {
  ...
  options: string[];
  answer: string;
  isTrick?: boolean; // If true, correct only when time runs out (no selection)
  trickMessage?: string; // Popup message when user picks an option on a trick question
}
...
const quizData: QuizQuestion[] = [
  ...
  },
  {
    ...
    answer: "__TRICK_TIMEOUT__", // No correct option — only timeout wins
    isTrick: true,
    trickMessage:
...
export default function Try() {
  ...
  // Ref to track if we're waiting for popup dismissal before advancing
  const waitingForPopup = useRef(false);
  // Ref for the current question to avoid stale closures
  ...
  const handleTimeout = useCallback(() => {
    ...
    if (q.isTrick) {
      // Trick question: timeout = correct! Player didn't pick any wrong answer
      setScore((s) => s + 1);
  ...
  const handleSelect = (option: string) => {
    ...
    if (q.isTrick) {
      // Trick question: any selection is wrong
  ...
  const handleTrickPopupClose = () => {
    ...
    // Advance to next question after popup is dismissed
  ...
  const handleShowLeaderboard = () => {
    // Leaderboard is already updated by useEffect when results state was reached
  ...
  // Determine the correct answer to show for the options component
  const getDisplayAnswer = () => {
    const q = quizData[currentQuestion];
    if (q.isTrick) {
      // For trick questions, there's no "correct" option to highlight
      return null;
    }
    return q.answer;
  };
              ...
              <AnimatePresence mode="wait">
                ...
                {gameState === "idle" && (
                  ...
                  >
                    ...
                    {/* Synthwave Loader as decorative element */}
                    <div className="my-4">
                      <SynthwaveLoader />
                    </div>
