        ...
        <ul className="hidden md:flex space-x-6 items-center">
          {navItems.map((item, idx) => (
            <li key={idx}>
              <Link
                to={item.to}
                className="relative group text-[#00ffff] hover:text-[#fff] transition-all duration-300 text-sm"
                style={{ fontFamily: "'Space Mono', monospace" }}
              >
                {item.label}
                <span className="absolute left-0 bottom-[-5px] w-full h-[2px] bg-[#00ffff] opacity-50 group-hover:opacity-100 transition-opacity"></span>
              </Link>
            </li>
          ))}
        </ul>
          ...
          {navItems.map((item, idx) => (
            <li key={idx}>
              <Link
                ...
                className="relative block text-lg font-bold text-[#00ffff] hover:text-[#fff] transition-all duration-300"
              >
                {item.label}
              </Link>
            </li>
